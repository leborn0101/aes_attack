...
# 获取第一轮攻击的度量分
def getfirstroundmeasurementscore():
    global plaintext
...
#  同上
def getfirstroundmeasurementscorebycache():
    global plaintext
    global allksvalue0
    result = []
    availsample = []
...
#  同上
def getfirstroundmeasurementscorebycache_nks():
    global plaintext
    global allksvalue0
    result = []
    availsample = []
    for i in range(16):
        ...
        for j in range(16):
            ...
            for k in range(samplenums):
                index = ((j * 16) ^ plaintext[k][i]) // 16
                # availsample[i][j].append(allksvalue0[k][(i % 4) * 16 + index])
                availsample[i][j].append(vessel[k][(i % 4) * 16 + index])
            result[i].append(measurementscore(availsample[i][j]))
    return result


# 获取第一轮攻击结果，以数组的形式返回，没个字节的后四位设为0
def getfirst4bits(frr):
    result = []
    for i in range(16):
        tmp = frr[i][0]
        key = 0
...
# 获取aes秘钥中索引位置为0、 5、 10、 15字节的后4位
def getsecondroundmeasurementscore0_5_10_15(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为4、 9、 14、 3字节的后4位
def getsecondroundmeasurementscore3_4_9_14(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为8、 13、 2、 7字节的后4位
def getsecondroundmeasurementscore2_7_8_13(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为12、 1、 6、 11字节的后4位
def getsecondroundmeasurementscore1_6_11_12(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为0、 5、 10、 15字节的后4位
def getsecondroundmeasurementscore0_5_10_15_nks(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为4、 9、 14、 3字节的后4位
def getsecondroundmeasurementscore3_4_9_14_nks(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为8、 13、 2、 7字节的后4位
def getsecondroundmeasurementscore2_7_8_13_nks(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
# 获取aes秘钥中索引位置为12、 1、 6、 11字节的后4位
def getsecondroundmeasurementscore1_6_11_12_nks(frresult):
    global vessel
    global samplenums
    global plaintext
    global allksvalue0
...
def getresult(r0, r1, r2, r3):
    r_0_5_10_15 = getrank(r0)
    r_4_9_14_3 = getrank(r3)
    r_8_13_2_7 = getrank(r2)
    r_12_1_6_11 = getrank(r1)
    global spth
    pt = spth + "/r_0_5_10_15"
    with open(pt, "w") as f:
...
def getresult_nks(r0, r1, r2, r3):
    r_0_5_10_15 = getrank(r0)
    r_4_9_14_3 = getrank(r3)
    r_8_13_2_7 = getrank(r2)
    r_12_1_6_11 = getrank(r1)
    global spth
    pt = spth + "/r_0_5_10_15_nks"
    with open(pt, "w") as f:
        for i in range(65536):
            t = r_0_5_10_15[i]
            if (t.i == 0 and t.j == 5 and t.k == 2 and t.l == 7):
                print("rank r_0_5_10_15_nks is %d\n" % i)
            s = "%3d%3d%3d%3d %10.8f" % (t.i, t.j, t.k, t.l, t.s)
            f.write(s)
            f.write("\n")
    pt = spth + "/r_4_9_14_3_nks"
    with open(pt, "w") as f:
        for i in range(65536):
            t = r_4_9_14_3[i]
            if (t.i == 4 and t.j == 1 and t.k == 6 and t.l == 3):
                print("rank r_4_9_14_3_nks is %d\n" % i)
            s = "%3d%3d%3d%3d %10.8f" % (t.i, t.j, t.k, t.l, t.s)
            f.write(s)
            f.write("\n")
    pt = spth + "/r_8_13_2_7_nks"
    with open(pt, "w") as f:
        for i in range(65536):
            t = r_8_13_2_7[i]
            if (t.i == 0 and t.j == 5 and t.k == 2 and t.l == 7):
                print("rank r_8_13_2_7_nks is %d\n" % i)
            s = "%3d%3d%3d%3d %10.8f" % (t.i, t.j, t.k, t.l, t.s)
            f.write(s)
            f.write("\n")
    pt = spth + "/r_12_1_6_11_nks"
    with open(pt, "w") as f:
        for i in range(65536):
            t = r_12_1_6_11[i]
            if (t.i == 4 and t.j == 1 and t.k == 6 and t.l == 3):
                print("rank r_12_1_6_11_nks is %d\n" % i)
            s = "%3d%3d%3d%3d %10.8f" % (t.i, t.j, t.k, t.l, t.s)
            f.write(s)
            f.write("\n")


def draw_hist(hit, miss, Title,Xlabel,Ylabel,Xmin,Xmax,Ymin,Ymax):
    plt.hist(hit,5000, facecolor='green')
    plt.hist(miss, 5000, facecolor='red')
    plt.xlabel(u"time")
    plt.xlim(Xmin,Xmax)
    plt.ylabel(u"counts")
    # plt.ylim(Ymin,Ymax)
...
def drawhitmiss(f):
    ...
    hit = next(g)
    miss = next(g)
    draw_hist(hit, miss, u"Perf", u"time", u"counts", 0, 5000, 0, 50000)