...
typedef void (*hit_function_t)(libflush_session_t* libflush_session, void*
    address,  size_t runs, size_t* histogram);
...
main(int argc, char* argv[])
{
  ...
  FILE* logfile = NULL;
  logfile = fopen("timelog", "w+");
  if (logfile == NULL) {
    fprintf(stderr, "Error: Could not open logfile '%s'\n", optarg);
    return -1;
  }
  hit_function_t hit_function = evict_reload_hit;
  miss_function_t miss_function = evict_reload_miss;
  size_t histogram_entries = HISTOGRAM_ENTRIES;

  /* Parse arguments */
  static const char* short_options = "f:c:t:n:z:h";
  ...
  /* Allocate histograms */
  size_t* hit_histogram = calloc(histogram_entries, sizeof(size_t));
  if (hit_histogram == NULL) {
    fprintf(stderr, "Error: Could not allocate memory for histogram.\n");
    return -1;
  }

  size_t* miss_histogram = calloc(histogram_entries, sizeof(size_t));
...
#define MAP_SIZE 4096
  ...
  for (unsigned int i = 0; i < histogram_entries; i++) {
    fprintf(logfile, "%zu ", hit_histogram[i]);
  }
  fprintf(logfile, "\n");
  for (unsigned int i = 0; i < histogram_entries; i++) {
    fprintf(logfile, "%zu ", miss_histogram[i]);
  }
...
static void
flush_reload_hit(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
...
static void
flush_reload_miss(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
...
static void
prime_probe_hit(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
...
static void
prime_probe_miss(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  size_t set_index = libflush_get_set_index(libflush_session, address);
...
static void
evict_reload_hit(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
    size_t time = libflush_reload_address(libflush_session, address);
    histogram[i] = time;
...
static void
evict_reload_miss(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
...
static void
flush_flush_hit(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
...
static void
flush_flush_miss(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
    size_t time = libflush_flush_time(libflush_session, address);
    histogram[i] = time;
...
static void
prefetch_hit(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
...
static void
prefetch_miss(libflush_session_t* libflush_session, void* address, size_t runs, size_t* histogram)
{
  for (unsigned int i = 0; i < runs; i++) {
    libflush_flush(libflush_session, address);
    size_t time = libflush_prefetch_time(libflush_session, address);
