...
void defense(libflush_session_t *libflush_session) {
  for (int i = 0; i < 512; i++) {
    libflush_eviction_prime(libflush_session, i);
  }
  while (1) {
    for (int i = 0; i < 512; i++) {
      libflush_eviction_t* eviction = (libflush_eviction_t*) libflush_session->data;
      for (int j = 0; j < 5; j++) {
        libflush_access_memory(eviction->congruent_address_cache[i].congruent_virtual_addresses[j]);
      }
    }
  }
}
